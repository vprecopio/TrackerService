...
$title_secction_ticket = 'Tickets Sin Asignar';
...
//se modifico todo el formulario miercoles a la madrugada
...
?>
//hasta aca para que aparezca el id
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                        ...
                        <tbody class="bg-white dark:bg-gray-800">
                            <?php foreach ($array as $value) : ?>
                                <?php if ($value->usuario_asignado === NULL) : ?>
                                    <tr>
                                        <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                            <span><?= SanitizeText::Output(Carbon::parse($value->ticket_fecha_creacion)->locale('es_ES')->isoFormat('LL')) ?></span>
                                        </td>
                                        <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                        ...
                                        <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                            <span><?= SanitizeText::Output($value->equipo_modelo) ?></span>
                                        </td>
                                        <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
...
<!-- se agrego este script para que recarge en la misma  -->
<script>
    function asignarTicket(idTicket, idUsuario) {
        // Realiza la solicitud AJAX
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/ticket/editarUsuarioAsignado", true);
